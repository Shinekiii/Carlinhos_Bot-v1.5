...
client.on('connected', () => {
    ...
    // Recuperar o contador de mortes do banco de dados
...
client.on('message', (channel, user, message, self) => {
    ...
    if (message == '!sal_igor') {
        client.say(channel, `qual foi meu mano, segue lá o the best player of the game de navinha, se não tá ligado né.... VAI TER QUE JOGAR AMOR PELUDO POR 2 DIAS SEGUIDOS, então se não quiser essa maldição segue lá pow: PowerUpL twitch.tv/salx_igormonteiro PowerUpR`);
    }

    //flowleme

    if (message == '!flowleme') {
        client.say(channel, `qual foi meu mano, segue lá o cara mais opium de socorro, se não tá ligado né.... VAI TER QUE ASSISTIR BORA BILL POR 24 HORAS, então se não quiser essa maldição segue lá pow: PowerUpL twitch.tv/flowleme PowerUpR`);
    }

    //crjunior

    if (message == '!crjunior') {
        client.say(channel, `qual foi meu mano, segue lá o GRAND MASTER em jogos retrô, se não tá ligado né.... VAI TER QUE JOGAR O JOGO DO ET DA ATARI POR 1 ANO, então se não quiser essa maldição segue lá pow: PowerUpL twitch.tv/crjuniortv PowerUpR`);
    }

    //kleber

    if (message == '!klebergames') {
        client.say(channel, `qual foi meu mano, segue lá o maior player de Pocket Bravery do multiverso, se não tá ligado né.... VAI TER QUE ASSISTIR INTERNET O FILME TODO DIA ATÉ O FINAL DA SUA VIDA, então se não quiser essa maldição segue lá pow: PowerUpL twitch.tv/kleber_gamesgg PowerUpR`);
    }

    //andergroundz

    if (message == '!andergroundz') {
        client.say(channel, `qual foi meu mano, segue lá o maior apresentador do gamesgia de todos os tempos, se não tá ligado né.... VAI BATER O DEDO NA QUINA POR 1 ANO TODOS OS DIAS SEGUIDOS, então se não quiser essa maldição segue lá pow: PowerUpL twitch.tv/andergroundz PowerUpR`);
    }

    //togurolins

    if (message == '!togurolins') {
        client.say(channel, `qual foi meu mano, segue lá o maior apreciador de suco de clorofila de todos os tempos, se não tá ligado né.... O TEU PC/CELULAR/CONSOLE VAI QUEIMAR, então se não quiser essa maldição segue lá pow: PowerUpL twitch.tv/togurolins PowerUpR`);
    }

    //steve blocos

    if (message == '!steveblocos') {
        client.say(channel, `qual foi meu mano, segue lá o maior vagabundo da twitch, se não tá ligado né.... VAI PERDER SEU CACHORRO NO MINECRAFT, então se não quiser essa maldição segue lá pow: PowerUpL twitch.tv/steveblocos PowerUpR`);
    }

    //husknou

    if (message == '!husknou') {
        client.say(channel, `qual foi meu mano, segue lá o maior furry sulista da twitch, se não tá ligado né.... VAI NAMORAR UMA MULHER CONFUSA, então se não quiser essa maldição segue lá pow: PowerUpL twitch.tv/husknou PowerUpR`);
    }

    //daphnim

    if (message == '!daphnim') {
        client.say(channel, `qual foi meu mano, segue lá a maior player de elsword de todos os tempos, se não tá ligado né.... VAI TER QUE USAR UM LG K10 PRO RESTO DA VIDA, então se não quiser essa maldição segue lá pow: PowerUpL twitch.tv/daphnim PowerUpR`);
    }
    ...
    // Comando para mostrar o contador de mortes atual
    ...
    if (message == '!steam') {
        client.say(channel, 'ID de Amigo : 110879874 https://steamcommunity.com/id/toddstr619/');
    }
    ...
    if (message == '!street') {
        client.say(channel, `@${user.username}, street ganha do fighter mas fighter ganha do 6`);
    }
    ...
    if (message == '!akuma') {
        client.say(channel, `@${user.username}, Site para o Frame Data do Akuma: https://wiki.supercombo.gg/w/Street_Fighter_6/Akuma. `);
    }
    ...
    //Frame data do M.Bison

    if (message == '!bison') {
        client.say(channel, `@${user.username}, Site para o Frame Data do M.Bison: https://wiki.supercombo.gg/w/Street_Fighter_6/M.Bison `);
    }
    //Frame data do Ken
    ...
    if (message == '!ed') {
        client.say(channel, `@${user.username}, Site para o Frame Data do Ed: https://wiki.supercombo.gg/w/Street_Fighter_6/Ed `);
    }
    ...
    const cooldowns = {}; // Objeto para armazenar os tempos de execução dos comandos
    const lastMessages = {}; // Objeto para armazenar a última mensagem processada por usuário

    client.on('message', (channel, tags, message, self) => {
        if (self) return; // Ignora mensagens próprias do bot

        const command = '!amor';
        const commandKey = `${channel}-${tags.username}-${command}`;
        const cooldownTime = 5000; // Tempo em milissegundos antes que o bot possa responder novamente

        // Verifica se o comando é !amor e se a última mensagem do usuário foi diferente
        if (message.toLowerCase().startsWith(command) && lastMessages[tags.username] !== message) {
            if (cooldowns[commandKey] && (Date.now() - cooldowns[commandKey]) < cooldownTime) {
                return; // Ignora o comando se estiver dentro do tempo de cooldown
            }

            const users = message.toLowerCase().split(`${command} `)[1].split(' e ');
            const lovePercentage = Math.floor(Math.random() * 101);
            const response = `A porcentagem de amor entre ${users[0]} e ${users[1]} é de ${lovePercentage}%! ❤️`;

            client.say(channel, response);

            cooldowns[commandKey] = Date.now(); // Registra o tempo de execução do comando
            lastMessages[tags.username] = message; // Registra a última mensagem processada pelo usuário
        }
    });
    ...
    client.on('message', (channel, tags, message, self) => {
        ...
        // Verifique se a mensagem foi recebida recentemente
        if (lastMessageTimestamp[username] && (currentTime - lastMessageTimestamp[username]) < DEBOUNCE_TIME) {
            return; // Ignore mensagens duplicadas em um curto período de tempo
        }

        // Atualize o timestamp da última mensagem
        ...
        if ((isModerator || isBroadcaster) && message.toLowerCase().startsWith('!addpontos')) {
            ...
            const targetUsername = splitMessage[1].toLowerCase(); // Converta para minúsculas para evitar inconsistências
                ...
                if (userChoice === correctCup) {
                    ...
                    db.get('SELECT * FROM points WHERE username = ?', username, (err, row) => {
                        ...
                        if (row) {
                            ...
                            db.run('INSERT INTO points (username, points) VALUES (?, ?)', [username, -10], (err) => {
                                if (err) {
                                    console.error(err.message);
                                } else {
                                    client.say(channel, `@${username}, que pena! Você errou e perdeu 10 pontos.`);
